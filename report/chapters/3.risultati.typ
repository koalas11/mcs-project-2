...
- *Tempo di esecuzione:* Come previsto dalla teoria, l’implementazione naive ha mostrato un andamento di complessità $O(n^3)$ per immagini di dimensione $n times n$, mentre l’implementazione FFTW ha confermato la complessità teorica di $O(n^2 log_2 n)$.
...
#figure(
  ...
  cetz.canvas({
    ...
    let min = calc.floor(-10)
    let max = calc.ceil(10)
...
#figure(
  caption: [Sinistra: immagine non compressa, $128"Kb"$. Destra: immagine compressa con $p = 10$ e $l=9$, 16Kb.],
  grid(columns: (1fr,) * 2, text(fill: red, size: 10em)[ciao], text(fill: red, size: 10em)[ciao]),
)
